...
async function SendRequestByProxy(method: string = 'GET', url: string | false = false, data: Record<string, string>, proxy: Object): Promise<any> {
  ...
  proxyUrl = `${proxyInstance.host}:${proxyInstance.port}`;
  ...
  if (proxyParts.length > 3) {
    ...
    proxyUrl = `http://${proxyParts[2]}:${proxyParts[3]}@${proxyUrl}`;
  }

  console.log(proxyUrl);
  // Tạo một instance của HTTP Proxy Agent
  const agent = ProxyAgent(proxyUrl);

  // Cấu hình Axios để sử dụng proxy
...
export const loginAccount = async function (accountInfo: string[], isBlock_1: bool = false, isBlock_2: bool = false) {
